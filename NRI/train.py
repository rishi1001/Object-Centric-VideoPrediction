...
def train(epoch, best_val_loss):
    ...
    decoder.train()
    scheduler.step()
    for batch_idx, (data, node_counts) in enumerate(train_loader):
        if args.cuda:
            data = data.cuda()
        
        # get indices where node_counts is chang from prev 
        node_counts_diff = []
        for i in range(len(node_counts)):
            if i == 0:
                node_counts_diff.append(0)
            elif node_counts[i] != node_counts[i-1]:
                node_counts_diff.append(i)
        
        for i in range(len(node_counts_diff)):
            data_ = data[node_counts_diff[i]:node_counts_diff[i+1]]
            ...
            logits = encoder(data_, rel_rec, rel_send)
            edges = gumbel_softmax(logits, tau=args.temp, hard=args.hard)
            prob = my_softmax(logits, -1)

            if args.decoder == 'rnn':
                output = decoder(data_, edges, rel_rec, rel_send, 100,
                                burn_in=True,
                                burn_in_steps=args.timesteps - args.prediction_steps)
            else:
                output = decoder(data_, edges, rel_rec, rel_send,
    ...
    encoder.eval()
    decoder.eval()
    for batch_idx, (data, node_counts) in enumerate(valid_loader):
        if args.cuda:
            data = data.cuda()
        ...
        for i in range(len(node_counts_diff)):
            ...
            data_ = Variable(data_, volatile=True)
            ...
            output = decoder(data_, edges, rel_rec, rel_send, 1)
...
def test():
    ...
    encoder.load_state_dict(torch.load(encoder_file))
    decoder.load_state_dict(torch.load(decoder_file))
    for batch_idx, (data, node_counts) in enumerate(test_loader):
        if args.cuda:
            data = data.cuda()
        # breakpoint()        # issue here
        ...
        for i in range(len(node_counts_diff)):
            ...
            data_encoder = data_[:, :, :args.timesteps, :].contiguous()
            data_decoder = data_[:, :, -args.timesteps:, :].contiguous()
