...
class MLP_OCVAEncoder(MessagePassing):
    def __init__(self, n_in, n_hid, n_out, do_prob=0., factor=True):
        super(MLP_OCVAEncoder, self).__init__()
    ...
    def edge2node(self, x, edge_index):
        # do message passing
        x = self.propagate(edge_index=edge_index, x=x)      
        return x

    def node2edge(self, x, edge_i, edge_j):
        # embedding for edges
        edges = torch.cat([x[edge_i], x[edge_j]], dim=1)
        return edges
    ...
    def forward(self, x, edge_index):
        ...
        x = self.node2edge(x, edge_i, edge_j)
        x = self.mlp2(x)
        x_skip = x

        if self.factor:
            x = self.edge2node(x, edge_index)
            x = self.mlp3(x)
            x = self.node2edge(x, edge_i, edge_j)
            x = torch.cat((x, x_skip), dim=2)  # Skip connection
            x = self.mlp4(x)
...
class MLP_OCVADecoder(nn.Module):
    """MLP decoder module."""

    def __init__(self, n_in_node, edge_types, msg_hid, msg_out, n_hid,
                 do_prob=0., skip_first=False):
        super(MLP_OCVADecoder, self).__init__()
        self.msg_fc1 = nn.ModuleList(
            [nn.Linear(2 * n_in_node, msg_hid) for _ in range(edge_types)])
        self.msg_fc2 = nn.ModuleList(
            [nn.Linear(msg_hid, msg_out) for _ in range(edge_types)])
    ...
    def forward(self, x,edge_index, rel_type, pred_steps=1):
        # NOTE: Assumes that we have the same graph across all samples.

        x = x.transpose(1, 2).contiguous()
        rel_rec = edge_index[1]
        rel_send = edge_index[0]

        sizes = [rel_type.size(0), x.size(1), rel_type.size(1),
                 rel_type.size(2)]
        rel_type = rel_type.unsqueeze(1).expand(sizes)
