...
parser.add_argument('--encoder', type=str, default='mlp_ocva',
                    help='Type of path encoder model (mlp or cnn).')
parser.add_argument('--decoder', type=str, default='mlp_ocva',
...
if args.encoder == 'mlp':
    ...
    encoder = CNNEncoder(args.dims, args.encoder_hidden,
                         args.edge_types,
                         args.encoder_dropout, args.factor)
elif args.encoder == 'mlp_ocva':
    encoder = MLP_OCVAEncoder(args.timesteps * args.dims, args.encoder_hidden,
                         args.edge_types,
                         args.encoder_dropout, args.factor)
...
def train(epoch, best_val_loss):
    ...
    for data in enumerate(train_loader):
        ...
        logits = encoder(data.x, data.edge_index)
        edges = gumbel_softmax(logits, tau=args.temp, hard=args.hard)
        prob = my_softmax(logits, -1)

        if args.decoder == 'rnn':
            output = decoder(data, edges, 100,
                             burn_in=True,
                             burn_in_steps=args.timesteps - args.prediction_steps)
        else:
            output = decoder(data.x,data.edge_index, edges,
...
def test():
    ...
    for data in enumerate(test_loader):
        ...
        output = decoder(data_decoder, edges, 1)
        ...
        if args.decoder == 'rnn':
            ...
            data_plot = data[:, :, args.timesteps:args.timesteps + 21,
                        :].contiguous()
            output = decoder(data_plot, edges, 20)
            target = data_plot[:, :, 1:, :]
